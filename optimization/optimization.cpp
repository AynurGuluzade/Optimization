...
namespace optimization
{
	...
	void optimize::GetDoorVisibilityPriors(){//get priors from the positive tree example we have
		...
		for (auto child : room->children)
		{
			if (child->getType() == "wall" && child->children.size() > 0)
			{
				for (auto door : child->children)
				{
					if (door->getType() == "door")
					{
						if (child->getName() == "fwall")
						{
	...
	double optimize::PriorDistanceCost(vector<shape> newFurnitures){//calculate prior distance cost
		...
		for (auto currentFur : newFurnitures){
			//if(newFurnitures[i].getPair() != "") continue;
	...
	void optimize::SimulatedAnnealing(){//main function for simulated annealing
		//generate vectors for centers, diagonals, prior distance
		vector<shape> furnitures;
		vector<shape> newFurnitures;
		vector<double> x, fx;//plotting temp graph
		double newTotalCost, currentTotalCost, transition_probability, rand; 
		double alpha = 0.9995; //constant number to decrease temperature at each iteration
		temperature = 1000.0; 
		double epsilon = 0.001; //lower temperature to acvhieve to stop iteration
		int iteration = 0;
		...
		{ 
			furnitures.push_back(fur.second);
		...
		x.push_back(0);
		fx.push_back(temperature);

		while(temperature > epsilon){//main loop//temperature
			...
			transition_probability = AcceptanceProbability(currentTotalCost, newTotalCost, temperature);
			rand = distribution(generator);//random number [0,1)
			if(transition_probability > rand){//if its true then we move to the new position
			...
			//decrase temperature/cooling down
			temperature *= alpha;
			...
			if (iteration % 5000 == 0){
				DebugSimAn(iteration, currentTotalCost, newTotalCost, temperature, transition_probability, rand);
